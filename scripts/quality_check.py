...
示例:
    python3 quality_check.py .claude/skills/elon-musk-skill/SKILL.md
"""
...
def check_mental_models(content):
    """检查心智模型数量（3-7个）"""
    # 匹配 ### 模型N: 或 ### N. 等模式
    ...
    if not models:
        ...
        if count > 0:
            passed = 3 <= count <= 7
            return passed, "{}个心智模型 {}".format(
                count, '✅' if passed else '❌ (应为3-7个)')
...
def check_expression_dna(content):
    """检查表达DNA辨识度"""
    ...
    return passed, "表达DNA特征: {}项 {}".format(
        style_markers, '✅' if passed else '❌ (应≥3项)')
...
def check_honest_boundary(content):
    ...
    count = len(items)
    passed = count >= 3
    return passed, "诚实边界: {}条 {}".format(
        count, '✅' if passed else '❌ (应≥3条)')


def check_tensions(content):
    """检查内在张力（至少2对）"""
    tension_markers = len(re.findall(r'张力|矛盾|tension|paradox|一方面.*另一方面|既.*又', content, re.IGNORECASE))
    passed = tension_markers >= 2
    return passed, "内在张力: {}处 {}".format(
        tension_markers, '✅' if passed else '❌ (应≥2处)')


def check_primary_sources(content):
    """检查一手来源占比"""
    # 找调研来源section
    ...
    ratio = primary / total
    passed = ratio > 0.5
    return passed, "一手来源占比: {}/{} ({:.0%}) {}".format(
        primary, total, ratio, '✅' if passed else '❌ (应>50%)')
...
def main():
    ...
    print("质量检查: {}".format(skill_path.name))
    print("=" * 50)
    ...
    for name, check_fn in checks:
        ...
        print("  {name:<12} {status}  {detail}".format(
            name=name, status=status, detail=detail))
        if passed:
            passed_count += 1

    print("=" * 50)
    print("结果: {}/{} 通过".format(passed_count, total))
