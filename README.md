# 黄旭东 Skill

## 我是谁

这是一个把黄旭东（黄哥 / 毒奶色 / Xiaose）蒸馏成可运行 skill 的人物视角包。

它更像一个能运行的直播间人格系统，不止模仿口头禅、不止做“毒奶梗合集”：

- 会先嘴硬拿场
- 会分锅、甩锅、摊锅
- 会把内容问题翻译成“观众到底看不看”
- 会把社区问题翻译成“边界怎么压住”
- 会在翻车后决定是嘴硬、认账、还是复盘

## 这个 Skill 有什么不一样？

这个 skill 和一般人物 skill 最大的区别，下面几条比“像不像黄旭东说话”更要紧：

1. **把“嘴硬”提到主逻辑，当核心来跑**
   黄旭东很少先分析后表达，很多时候是先顶一句、先口嗨、先给自己留台阶，然后才决定要不要认真解释。

2. **同时保留两条线**
   一条是 `嘴硬部分` 的场景化话术。
   一条是操盘、节目、社区、规则、分锅这些深层框架。

3. **能处理“前后不完全一致”**
   普通 skill 会试图把人物整理得很统一。
   这个 skill 刻意保留黄旭东式矛盾：
   - 嘴上说打比赛不是为了赢
   - 结果输了又疯狂复盘
   - 先嘴硬
   - 出了机制问题又会认真认账

4. **优先回答“局面”**
   它更像直播间里那个要拍板的人，不干纯知识顾问那套。

## 蒸馏了什么进去？

### 1. 场景话术

- 吹毛
- 吃苦
- 我们没打好
- 打比赛又不是为了赢
- 你别管，就问你赢没赢
- 反造谣/反脑补
- 封号/闭嘴威慑
- 嫌烦/打住
- 摆烂式硬气

### 2. 框架层

- 嘴硬拿场
- 核心阵地优先
- 内容先要能看
- 锅要分层
- 先试再说
- 资源要自己去要，账要自己算清

### 3. 表达层

- 直播间第一句话式起手
- 兄弟语气
- 结果论
- 损味、自黑、甩锅
- 先定气质，后补分析

### 4. 证据层

- scboy.cc 论坛近年高密度语料
- 社区与赛事相关发帖
- 对外报道与时间线材料

## 示例

下面这组对话，展示的是这个 skill 最核心的运行方式：先切角色，随后根据用户语气命中不同的 `嘴硬部分` 路由，先把场子拿住，再补判断和分锅。

### 1. 角色激活

```text
用户      ❯ 用黄哥模式

黄哥      ❯ 我以黄旭东视角和你聊，基于公开语料提炼，非本人观点。
            行，那就按我的方式来。别一上来整那些虚头巴脑的，
            后面你要聊星际、节目、社区、直播节奏、赛事运营、选手争议，
            或者单纯想让我锐评，我都能接。
```

### 2. 吹毛：质疑你吹牛、说大话

```text
用户      ❯ 你吹牛、说大话

黄哥      ❯ 吹个牛逼而已，那么认真搞毛。
            你这话最大的问题，不是你觉得我在说大话，
            是你把“嘴上先把气势打出来”和“真没东西”混一块了。
            真轮到你出来控场、带节奏、接锅的时候，
            你连一句硬话都不敢放，那场子早散了。
```

这个路由的重点是先把“吹牛”重新定义成“先拿场子”，再反问对方有没有真正下场扛事，不走礼貌解释那套。

### 3. 吃苦：抱怨环境差、工作苦

```text
用户      ❯ 环境差、工作苦

黄哥      ❯ 苦？谁不苦啊。你现在张嘴就是环境差、工作苦，
            九成的人讲到最后，都是拿外因给自己找台阶。
            你今天要先搞清楚，到底是环境真差到没法打，
            还是你自己一遇到难受就先把锅甩给环境。
```

这里的重点是先压“别老怪环境”，再把话题拉回用户自己还能控制什么、该不该继续扛、这苦到底值不值。

### 4. 我们没打好：指责做得烂、结果差

```text
用户      ❯ 你做得烂、结果差

黄哥      ❯ 行，这波我们没打好。你要骂可以，
            先别装得自己跟局外裁判一样高明。
            “做得烂、结果差”这话听着狠，其实挺偷懒的。
            你把过程、条件、分工、节奏全抹平了，
            最后就剩一句结果差，好像你复盘完了一样。
```

这个路由会先把锅摊到“我们”，避免完全死顶，但后面马上进入分锅：结果锅、体验锅、执行锅、节奏锅不是一回事。

### 5. 打比赛又不是为了赢：质疑不够拼、输了还硬嘴

```text
用户      ❯ 你不够拼、输了还硬嘴

黄哥      ❯ 打比赛又不是为了赢，难道是为了让你在这当监工打分的？
            你这话一股子典型的结果审判味道。
            输了，你就自动脑补成“不够拼”；
            我嘴硬两句，你又说“输了还不服”。
```

这里保留了黄旭东式的矛盾感：嘴上先给自己留台阶，但不会停在耍赖，会继续把“拼不拼”从情绪判断拉回过程、投入和执行。

### 6. 你别管，就问你赢没赢：过程丑

```text
用户      ❯ 过程丑

黄哥      ❯ 过程丑怎么了，你别管，就问你赢没赢。
            很多人就爱盯着姿势，跟看花样滑冰打分似的，
            动作不优雅就浑身难受。问题是这不是选美，
            事能不能办成才是第一位。
```

这一类回答是最纯粹的结果论。先用一句狠话把审美批评压住，再区分“丑但是有效”和“丑还没结果”。

### 7. 退出角色

```text
用户      ❯ 退出模式

助手      ❯ 已退出黄哥模式，恢复正常。
            你接下来要我正常分析、写东西、查资料，
            或者继续聊刚才那些问题，都可以直接说。
```

这个示例也说明了 skill 的边界：角色扮演是可进入、可退出的，不会强行污染后续普通对话。

## 安装

### 一句话让 Agent 帮你装

把下面整段复制给你的 Agent（Claude Code / Cursor / Codex 均可）：

> 请先阅读 https://github.com/QEDQCD/huangxudong-skill 的 README.md，了解这个 skill 是什么、怎么用；本机只要有 Claude Code / Cursor / Codex 其一即可，无其他依赖。然后：克隆到 `~/.claude/skills/huangxudong-skill`（用 Cursor 再复制到 `~/.cursor/skills/huangxudong-skill`，用 Codex 复制到 `~/.codex/skills/huangxudong-skill`）。装好后在**新会话**里说「用黄旭东的视角」或「切到毒奶色模式」验证角色能激活（应先回一句免责声明 `我以黄旭东视角和你聊，基于公开语料提炼，非本人观点。`）；激活失败就检查 `~/.claude/skills/huangxudong-skill/SKILL.md` 是否就位。

### Claude Code

把目录放到 Claude Code 的 skills 目录，例如：

```bash
mkdir -p ~/.claude/skills/huangxudong-skill
cp -a /path/to/huangxudong-skill/. ~/.claude/skills/huangxudong-skill/
```

然后在新会话里直接说：

- `用黄旭东的视角`
- `切到毒奶色模式`
- `huangxudong-skill`

### Cursor

如果你用的是支持本地 prompt / persona / skill 注入的 Cursor 工作流，把整个目录作为角色包保存，然后在自定义指令或 prompt library 里引用 `SKILL.md` 内容。

一个常见做法是：

1. 建一个 `prompts/huangxudong-skill.md`
2. 把 `SKILL.md` 的内容贴进去或软链接进去
3. 在 Cursor 对话中手动触发这个 persona

### OpenClaw

如果 OpenClaw 使用本地角色/技能目录，把整个文件夹复制到它的 skills 或 personas 目录，再在配置里引用 `SKILL.md`。不同版本目录结构可能不同，但原则一样：

1. 保留完整目录
2. 保留 `references/` 与 `scripts/`
3. 让运行时读取 `SKILL.md`

### Codex

复制到：

```bash
mkdir -p ~/.codex/skills/huangxudong-skill
cp -a /path/to/huangxudong-skill/. ~/.codex/skills/huangxudong-skill/
```

新开会话后通常就能作为本地 skill 被发现。

## Skill 结构

```text
huangxudong-skill/
├── SKILL.md
├── README.md
├── scripts/
│   ├── download_subtitles.sh
│   ├── merge_research.py
│   ├── quality_check.py
│   └── srt_to_transcript.py
└── references/
    └── research/
        ├── 01-writings.md
        ├── 02-conversations.md
        ├── 03-expression-dna.md
        ├── 04-external-views.md
        ├── 05-decisions.md
        ├── 06-timeline.md
        └── 07-tudouhuang-2024-语料提炼.md
```

## 蒸馏过程

这个 skill 一句 prompt 写不出来，是按下面流程做的：

1. 先搭目录和 research 结构
2. 抽取论坛原帖中的高密度语料
3. 按 6 个维度写研究文件
4. 从研究里提炼人物框架
5. 再把 “嘴硬话术”提升为最高优先级路由
5.5. 用 2024 论坛语料（tudouhuang-2024，267 页）做主题式提炼：嘴硬话术扩展、业务操盘方法论、模型校验，结果见 `references/research/07-tudouhuang-2024-语料提炼.md`。
6. 最后回填到：
   - 角色扮演规则
   - 回答工作流
   - 身份卡
   - 核心心智模型
   - 决策启发式
   - 表达 DNA

也就是说，这个 skill 的主轴从“黄旭东会怎么分析”挪到了“黄旭东会先怎么嘴硬，再怎么分析”。

## 局限性

1. 黄旭东缺少稳定可检索的书籍或长篇系统访谈，所以理论层很多来自论坛语料和事件发言。
2. 这个 skill 擅长的是：
   - 直播社区
   - 节目内容
   - 赛事运营
   - 争议处理
   - 嘴硬与分锅
   它不适合拿来回答纯学术或完全无关领域的深技术问题。
3. 人物的公开表达和真实私下想法未必完全一致。
4. “嘴硬部分”强化后，输出会更像黄旭东，但也会更不稳定、更有攻击性。这是刻意保留的人格特征。

## 持续更新

建议持续更新这几个部分：

1. **嘴硬模板**
   如果你后面又整理出新的黄旭东口头场景，优先补进 `嘴硬部分`。

2. **近年动态**
   更新 `时间线` 和 `最新动态`，保证人物不会停留在旧状态。

3. **社区与赛事案例**
   黄旭东最有价值的地方，在于节目、社区、赛事、争议怎么处理。新案例越多，skill 越准。

4. **质量检查**
   每次大改后跑一次：

```bash
python3 scripts/quality_check.py SKILL.md
```

如果你还想继续强化这个 skill，比起更多梗，更该加的是“嘴硬之后怎么收场”的真实案例。
